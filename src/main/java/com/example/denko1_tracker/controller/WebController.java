...
public class WebController {
    ...
    public ResponseEntity<byte[]> downloadWrittenCsv(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        // UTF-8 BOM for Excel
        sb.append("\uFEFF");
        sb.append("年度,期,回数,計算,機器,施工図,法令,合計\n");
        for (WrittenExamRecord r : records) {
            sb.append(String.format("%s,%s,%d,%d,%d,%d,%d,%d\n",
                r.getExamYear(), r.getExamPeriod() != null ? r.getExamPeriod() : "", r.getAttemptNumber(), r.getCalcScore(), r.getMemoryScore(), r.getDiagramScore(), r.getLawScore(), r.getTotalScore()));
        }
