...
### 1. 筆記試験（過去問）管理 ＆ 弱点分析
- **分野別記録**: 過去問の得点を「計算」「機器」「施工図」「法令」の4つの重要カテゴリに分けて記録。
- **合計表示と視覚的判定**: 100点満点中の合計点を自動計算し、**合格ライン（60点）に達しない場合は数字を赤字で表示**。合格可否を瞬時に判断可能.
- **レーダーチャート分析**: 蓄積されたデータから自動的に正答率を算出し、チャートで視覚化。
- **最優先復習アラート**: 最も正答率が低い分野を「現在の弱点」として自動的に特定し、重点的な学習を促します。
...
### 4. 試験カウントダウン機能
- **目標日の可視化**: 学科・技能試験の目標日を設定し、ダッシュボードで「あと何日」かを可視化することで学習の計画性と意欲を向上させます。

### 5. 学習フェーズに合わせた表示最適化（技能試験集中モード）
- **表示切り替え機能**: 学科試験合格後など、不要になった筆記試験の情報（レーダーチャート、スコア履歴）を、ユーザー設定によりダッシュボードから非表示にできます。
- **UXの向上**: ユーザーが現在の目標に集中できるよう、画面上のノイズを減らし、最適な学習環境を提供します。

### 6. セキュアな個人データ管理 ＆ モバイル最適化
